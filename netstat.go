...
var inodeHash = make(map[string]string)
...
func init() {
	...
	re := regexp.MustCompile(`socket:\[(?P<loginToken>[0-9]*)\]`)
	for _, item := range d {
		path, _ := os.Readlink(item)
		ee := re.FindStringSubmatch(path)
		if len(ee) == 2 {
...
func netstat(t string) []Process {
	...
	data := getData(t)
	l := 10
	pp := make(chan Process, 4096)
	chaHash := make(map[int]chan string)
	for i := 0; i < l; i++ {
		chaHash[i] = make(chan string, 4096)
		go do(chaHash[i], pp, &l)
	}

	for v, line := range data {
		chaHash[v%l] <- line
	}
	for _, v := range chaHash {
		close(v)
	}
	for v := range pp {
...
func do(st <-chan string, send chan Process, l *int) {
	for line := range st {
		_ = line
		// local ip and port
		...
		senRecv := strings.Split(line_array[4], ":")
		rec := hexToDec(senRecv[1])
		sen := hexToDec(senRecv[0])
		uid := getUser(line_array[7])
		pid := findPid(line_array[9])
		exe := getProcessExe(pid)
		name := getProcessName(exe)
		p := Process{uid, name, pid, exe, state, ip, port, fip, fport, rec, sen}
		send <- p
	}
